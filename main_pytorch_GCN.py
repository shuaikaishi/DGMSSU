...
from model.model_GCN import EncoderGCN
...
for i in range(1, nums + 1):
    ...
    data_ = np.concatenate([data_, np.zeros([padding_l, Channel])])
...
train_L = torch.tensor(L, dtype=torch.float32).to(device)
train_mask = torch.tensor(mask, dtype=torch.float32).to(device)
train_db = torch.utils.data.TensorDataset(train_data, train_L, train_mask)
train_db = torch.utils.data.DataLoader(train_db, batch_size=batchsz, shuffle=True, drop_last=True)

################################################################
## define model and optimizer
encoder = EncoderGCN(P, Channel, z_dim, seq_l).to(device)
...
if training == True:
    ...
    for epoch in range(start_epoch, start_epoch + epochs):
        ...
        for step, data in enumerate(train_db):
            y_ = data[0].to(device)  # [B,N,C+2]
            graph_l = data[1].to(device)
            mask = data[2].to(device)

            y = y_[:, :, 0: -2]
            mu, log_var, a = encoder(y, graph_l, mask)
            std = (log_var * 0.5).exp()
            ...
            y_hat = a @ em_tensor
            mask_ = mask @ torch.ones(1, y_hat.shape[-1], device=mask.device)
            y_hat = y_hat * mask_
        ...
        if (epoch + 1) % (epochs // 10) == 0:
            ...
            with torch.no_grad():
                ...
                mu, log_var, a = encoder(train_data_, train_L, train_mask)  # [B,n,P]
                std = (log_var * 0.5).exp()
                eps = torch.randn(mu.shape, device=std.device)
                z = mu + eps * std
            ...
            print('save results!')
            toc = time.time()
            print('time elapsed:', epoch, toc - tic)
    ...
    with torch.no_grad():
        ...
        y = y.repeat(100, 1, 1)
        mu, log_var, a = encoder(y,train_L[101:102].repeat(100, 1, 1), train_mask[101:102].repeat(100, 1, 1))
        std = (log_var * 0.5).exp()
        ...
        scio.savemat('dgmgcn.mat', {'em_gen': em_gen})
toc = time.time()
...
with torch.no_grad():
    train_data_ = train_data[:, :, 0:-2]
    mu, log_var, a = encoder(train_data_, train_L, train_mask)
    std = (log_var * 0.5).exp()
    eps = torch.randn(mu.shape, device=std.device)
    z = mu + eps * std
...
######################################
# Visualization of attention maps (The renormalized Graph
# Laplacian for GCN).
data = L[0]
print(data.shape)
